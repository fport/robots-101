...
<text x="15" y="307" fill="#c1d2a7" font-size="11">DÜZLEMSEL FK · L₁ 18 cm · L₂ 14 cm · fizik ve çarpışma (collision) yok</text>
</svg>
...
**Deney:** kolu düzleştirdiğinde erişim 32 cm olur. Katladığında uç tabana yaklaşır. Aynı uç noktasına farklı eklem (joint) açılarından ulaşma fikrini düşün; IK'nin neden tek bir cevap vermeyebildiğini açıkla. Bu çizim eklem/masa çarpışmalarını hesaba katmaz.

## 2. Ne kadar veri toplayacaksın?

640×480 RGB görüntü varsayımıyla ham depolamayı ve net gösterim (demonstration) süresini hesapla. İki kamera, eylem (action) satırlarını ikiye katlamaz; her zaman adımında iki ayrı görüntü olur.

<div class="lab-panel" id="dataset-planner">
<div class="lab-controls">
<label for="episodes">bölüm (Episode) sayısı<input type="number" id="episodes" min="1" value="50"></label>
...
Bir VLA her model çağrısında birden fazla gelecek eylem tahmin edebilir. Diziden kaç eylem yürüttüğün, yeniden ne zaman gözlem (observation) aldığını etkiler.
...
**Deney:** 30 Hz'de 50 eylem yaklaşık 1,67 saniyelik pencere demektir. O sürede nesne kayarsa eski tahminler hâlâ yürütülüyor olabilir. Daha kısa pencere daha sık geri besleme isteyebilir; çıkarım (inference) bunu yetiştiremiyorsa eylem kuyruğu boşalabilir. Bu hesap gerçek bir RTC uygulaması değildir; gecikme (latency) bütçesini anlamaya yarar.
