...
Komutu [SmolVLA eğitim sayfasından](smolvla.md) üretebilirsin. Burada o komutun kurduğu problemi açıyoruz: görüntü, metin ve robot durumundan gelecekteki eylem (action) dizisi üretmek. Açıklama, kurulu **LeRobot 0.6.1** kodu ve ayrıca incelenen kaynak sürümü üzerinden hazırlanmıştır. kontrol noktası (Checkpoint) config'i bazı varsayılanları değiştirebilir; kendi kaydedilmiş config'in çalışma için esas kaynaktır.

## 1. Üç girdi, bir eylem dizisi

```mermaid
flowchart TD
    accTitle: SmolVLA girdilerinden gelecekteki eylemlere
    accDescr: Kamera görüntüleri, görev metni ve ölçülen durum koşullandırılmış temsili oluşturur. Eylem uzmanı bu temsil, gürültülü eylem dizisi ve akış zamanından düzeltme alanı tahmin eder. Akış çözümü gelecekteki eylemleri üretir.
    I["Kamera görüntüleri<br/>Görüntü özellikleri (visual features)"] --> R["Koşullandırılmış temsil<br/>(conditioned representation)"]
    T["Görev metni<br/>Dil tokenları (language tokens)"] --> R
    S["Ölçülen eklem durumu<br/>Durum projeksiyonu (state projection)"] --> R
    R --> E["Eylem uzmanı (action expert)"]
    N["Gürültülü eylem dizisi<br/>(noisy action chunk)<br/>+ akış zamanı (flow time)"] --> E
    E --> V["Eylem düzeltme alanı<br/>(action velocity field)"]
    V --> F["Gürültüden eyleme çözüm<br/>(flow integration)"]
    F --> A["Gelecekteki eklem hedefleri<br/>(future joint targets)"]
```
...
“Kırmızı küpü al” metni tek başına küp koordinatı sağlamaz; görüntü, durum (state) ve eğitimdeki örnek ilişkiler gerekir. Her bölüm (episode)'a aynı metni yazdıysan farklı nesne seçme talimatlarını öğrendiğini yalnız bu veriyle göstermiş olmazsın.

## 2. Tensörleri sayılarla okuyalım

Örnek ayar: örnek grubu (batch) `B=8`, iki kamera, kayıt çözünürlüğü 480×640, SO-101 action boyutu `D=6`, ufuk `H=50`.

| Aşama | Şekil örneği | Açıklama |
|---|---|---|
| Bir kamera, disk üstveri (metadata) | `[480,640,3]` | Yükseklik, genişlik, renk |
| Bir kamera, batch | `[8,3,480,640]` | Batch, kanal, yükseklik, genişlik |
| State | `[8,6]` | Karar anındaki eklem (joint)/tutucu (gripper) durumu |
| Action hedefi | `[8,50,6]` | 50 gelecekteki altı boyutlu komut |
| Zaman doldurma (padding) maskesi | `[8,50]` | Episode dışına taşan etiketler |
...
Flow matching'de eğitim örneğinin eylem dizisi ile rastgele gürültü arasında ara örnekler kurulur. Model, koşullara bakarak bu uzaydaki yön/hız (velocity) alanını öğrenir. Bu sayfadaki `t`, 30 Hz robot zamanı veya episode kare (frame) indeksi değildir; **veri ile gürültü arasındaki akış parametresidir**.
...
`num_steps=10`, “robot on adım hareket eder” demek değildir; bir eylem dizisi (action chunk) üretirken çözücünün kaç güncelleme yaptığıdır. `chunk_size=50` üretilen robot komutu sayısını belirler. Bu iki sayıyı karıştırma.

## 7. Bir eğitim güncellemesi

Bir batch seçilir; görüntüler/durum/metin hazırlanır ve gelecekteki action etiketleri toplanır. Gürültü ve akış zamanı örneklenir. Ağ tahmin üretir. Geçersiz episode adımları loss'tan maskelenir. Backward ile eğitilebilir parametrelerin gradyanları (gradients) hesaplanır; eniyileyici (optimizer) ağırlıkları değiştirir; öğrenme oranı zamanlayıcısı (scheduler) öğrenme oranını günceller. Kurulu eğitici gradyan (gradient) kırpma da uygular. [LeRobot eğitim döngüsü](https://github.com/huggingface/lerobot/blob/2774d9bddcbbda50e697e162e89e7eaada8d7105/src/lerobot/scripts/lerobot_train.py)
...
Aynı veri kümesi (dataset) örneğine farklı gürültü/t verilince training loss değişebilir. Bir batch'in loss'unu başka batch ile doğrudan görev başarısı gibi yorumlama. Normalizasyon (normalization) ve padding kapsamı değişirse loss ölçeği de değişebilir.
...
50 komut ürettiğinde hepsini 30 Hz'de çalıştırmak yaklaşık 1.67 s sürer. Bu süre boyunca yeni politika (policy) çıkarımı yapmayan bir uygulama, komutları eski gözleme göre yürütür. Daha kısa yürütme ufku sık yeniden planlamaya izin verebilir; fakat model gecikmesi yüksekse kuyruk boşalabilir.
